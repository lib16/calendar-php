...
class Calendar
{
     ...
     *            First day which should be shown on the calendar.<br>
     *            A string of the format <code>Y-m-d</code> or a
     *            <code>DateTime</code> object.
     * @param DateTime|string $till
     *            Last day which should be shown on the calendar.
     */
     ...
     *
     * @param int $count
     *            <= -2: The previous <code>$count</code> months
     ...
     *
     * @param string $format
     *            <code>%b</code>, <code>%B</code> or <code>%m</code>,
     ...
     *
     * @param string $format
     *            <code>%Y</code>, <code>%y</code> or an empty string.
     */
     ...
     *            Either a format string containing <code>%a</code> or
    ...
    {
        $this->firstWeekday = is_string($firstWeekday) ? self::firstWeekday($firstWeekday) : $firstWeekday;
        $this->buildWeekdays = true;
        return $this;
    }
    ...
    private static function firstWeekday($countryCode): int
    {
        ...
        foreach ($territories as $i => $territory) {
            if (in_array($countryCode, $territory)) {
                $firstWeekday = $i;
                break;
            }
        }
        ...
        if ($this->buildWeekdays) {
            ...
            for ($i = 0; $i < 7; $i ++) {
                ...
                if (\is_array($this->weekdayFormat)) {
                    ...
                    $this->weekdays[$index] = $day->formatLocalized($this->weekdayFormat);
                }
                $day->addDays(1);
            }
